...
// ---------- ข้อมูลคู่มือบริหาร (ทั่วไป ไม่ผูกแผนก) ----------
$stmt_regulations = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'regulations' ORDER BY sort_order ASC, id ASC");
$regulations_rows = $stmt_regulations->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-lg-9 col-xl-8" id="generalContent">
            <?php if (!empty($regulations_rows)): ?>
                <div class="d-flex flex-column gap-4">
                <?php foreach ($regulations_rows as $row):
                    $body = !empty($row['content']) ? nl2br(htmlspecialchars($row['content'])) : '';
                    $created = dateToThaiFull($row['created_at'] ?? '');
                    $attach  = renderAttachments($row);
                ?>
                <div class="dept-content-card">
                    <h2 class="dc-title"><?= htmlspecialchars($row['title']) ?></h2>
                    <div class="dc-meta mb-3"><i class="bi bi-calendar3 me-1"></i> <?= $created ?></div>
                    <?php if($body): ?><div class="dc-body mb-4"><?= $body ?></div><?php endif; ?>
                    ...
                    <?php endif; ?>
                    <?php if($attach): ?><div class="dc-attachments"><?= $attach ?></div><?php endif; ?>
                </div>
                <?php endforeach; ?>
                </div>
            <?php else: ?>
