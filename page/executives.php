...
// ทำเนียบหัวหน้ากลุ่มงาน — Grid รูป+ชื่อ+ตำแหน่ง เรียงตามลำดับความสำคัญ (sort_order)
// มี id แผนก -> โชว์ทำเนียบของแผนกนั้นเท่านั้น, ไม่มี id -> โชว์ทำเนียบภาพรวมองค์กร
if ($DEPT_ID !== null) {
    $stmt_roster = $conn->prepare("SELECT * FROM department_contents WHERE department_id = :id AND section = 'executives' ORDER BY sort_order ASC, id ASC");
    $stmt_roster->execute([':id' => $DEPT_ID]);
} else {
    $stmt_roster = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'executives' ORDER BY sort_order ASC, id ASC");
}
...
<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-12" id="generalContent">
            <?php if (!empty($rosterRows)): ?>
                <?php
                    // จัดกลุ่มตามตำแหน่ง แต่ละตำแหน่งขึ้นแถวใหม่ จัดกึ่งกลางแยกแถว (ไม่ไหลรวมกันทุก 4 คนแบบเดิม) ตามที่สั่ง
                    $rosterGroups = [];
