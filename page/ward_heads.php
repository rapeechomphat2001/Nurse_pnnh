...
// ทำเนียบหัวหน้างาน — Grid รูป+ชื่อ+ตำแหน่ง เรียงตามลำดับความสำคัญ (sort_order)
// มี id แผนก -> โชว์ทำเนียบของแผนกนั้นเท่านั้น, ไม่มี id -> โชว์ทำเนียบภาพรวมองค์กร
if ($DEPT_ID !== null) {
    $stmt_roster = $conn->prepare("SELECT * FROM department_contents WHERE department_id = :id AND section = 'ward_heads' ORDER BY sort_order ASC, id ASC");
    $stmt_roster->execute([':id' => $DEPT_ID]);
} else {
    $stmt_roster = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'ward_heads' ORDER BY sort_order ASC, id ASC");
}
