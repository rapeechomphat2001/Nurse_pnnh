...
// เฉพาะ dept user ของแผนกนี้เท่านั้น — ต้อง role='dept' และ department_id ตรงกับ id ใน URL
if ($DEPT_ID !== null) {
    if ($_SESSION['role'] !== 'dept' || (int)$_SESSION['department_id'] !== $DEPT_ID) {
        session_unset();
        session_destroy();
        header('Location: login.php?redirect=' . urlencode($_SERVER['REQUEST_URI']));
        exit;
...
// ---------- เนื้อหาทั่วไป ----------
$gc_rows = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'risk_management' ORDER BY sort_order ASC, id ASC")->fetchAll(PDO::FETCH_ASSOC);
?>
<!DOCTYPE html>
<html lang="th">
<head>
...
<div class="top-bar">
    <div class="container d-flex justify-content-between align-items-center flex-wrap gap-2">
        ...
        <a href="logout.php" class="btn btn-sm btn-outline-light"><i class="bi bi-box-arrow-right me-1"></i>ออกจากระบบ</a>
    </div>
</div>
...
<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-lg-9 col-xl-8" <?= $DEPT_ID === null ? 'id="generalContent"' : '' ?>>
            <?php if ($DEPT_ID !== null && !empty($bySection['risk_management'])): ?>
                <div class="d-flex flex-column gap-4">
                    <?php foreach ($bySection['risk_management'] as $row): ?>
                        <div class="dept-content-card">
                            <h2 class="dc-title"><?= htmlspecialchars($row['title']) ?></h2>
                            <div class="dc-meta"><i class="bi bi-calendar3 me-1"></i> <?= dateToThaiFull($row['created_at']) ?></div>
                            <?php if (!empty($row['content'])): ?>
                                <div class="dc-body mb-4"><?= nl2br(htmlspecialchars($row['content'])) ?></div>
                            <?php endif; ?>
                            <?php if (!empty($row['link_url'])): ?>
                                <div class="mb-3"><a href="<?= htmlspecialchars($row['link_url']) ?>" target="_blank"><i class="bi bi-link-45deg"></i> <?= htmlspecialchars($row['link_url']) ?></a></div>
                            <?php endif; ?>
            ...
            <?php else: ?>
            <?php if (!empty($gc_rows)): ?>
                <div class="d-flex flex-column gap-4">
                <?php foreach ($gc_rows as $row): ?>
                <div class="dept-content-card">
                    <h2 class="dc-title"><?= htmlspecialchars($row['title']) ?></h2>
                    <div class="dc-meta mb-3"><i class="bi bi-calendar3 me-1"></i> <?= dateToThaiFull($row['created_at'] ?? '') ?></div>
                    <?php if(!empty($row['content'])): ?><div class="dc-body mb-4"><?= nl2br(htmlspecialchars($row['content'])) ?></div><?php endif; ?>
                    <?php if(!empty($row['link_url'])): ?><div class="mb-3"><a href="<?= htmlspecialchars($row['link_url']) ?>" target="_blank"><i class="bi bi-link-45deg"></i> <?= htmlspecialchars($row['link_url']) ?></a></div><?php endif; ?>
                    <div class="dc-attachments"><?= renderAttachments($row) ?></div>
                </div>
                <?php endforeach; ?>
                </div>
