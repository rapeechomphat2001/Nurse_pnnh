...
// รูปบุคลากร — Grid รูป+ชื่อ+ตำแหน่ง เรียงตามลำดับความสำคัญ (sort_order)
// มี id แผนก -> โชว์เฉพาะแผนกนั้น, ไม่มี id -> โชว์ภาพรวมองค์กร
if ($DEPT_ID !== null) {
    $stmt_roster = $conn->prepare("SELECT * FROM department_contents WHERE department_id = :id AND section = 'personnel_gallery' ORDER BY sort_order ASC, id ASC");
    $stmt_roster->execute([':id' => $DEPT_ID]);
} else {
    $stmt_roster = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'personnel_gallery' ORDER BY sort_order ASC, id ASC");
}
$rosterRows  = $stmt_roster->fetchAll(PDO::FETCH_ASSOC);
...
<head>
    ...
    <title>รูปบุคลากร - กลุ่มงานการพยาบาล โรงพยาบาลปากช่องนานา</title>
...
<div class="container my-5">
    <div class="row justify-content-center">
        <div class="col-12" <?= $DEPT_ID === null ? 'id="generalContent"' : '' ?>>
            <?php if (!empty($rosterRows)): ?>
                <?php
                    // จัดกลุ่มตามตำแหน่ง แต่ละตำแหน่งขึ้นแถวใหม่ จัดกึ่งกลางแยกแถว
                    $rosterGroups = [];
                    foreach ($rosterRows as $row) { $rosterGroups[$row['content'] ?? ''][] = $row; }
                ?>
                <?php foreach ($rosterGroups as $positionRows): ?>
                <div class="row justify-content-center g-4 mb-4">
                    <?php foreach ($positionRows as $row):
                        $photo = parseFileNames($row['file_name'] ?? '');
                        $photo = $photo[0] ?? '';
                    ?>
                        <div class="col-6 col-md-3">
                            <div class="dc-personnel">
                                <img src="../uploads/<?= htmlspecialchars($photo) ?>" class="dc-img lightbox-trigger" alt="<?= htmlspecialchars($row['title']) ?>" onerror="this.style.display='none'">
                                <div class="fw-bold mb-1"><?= htmlspecialchars($row['title']) ?></div>
                                <span class="dc-personnel-role"><?= htmlspecialchars($row['content'] ?? '') ?></span>
                            </div>
                        </div>
                    <?php endforeach; ?>
                </div>
        ...
        <p>หน้า "รูปบุคลากร" อยู่ระหว่างการจัดทำ</p>
