...
// ---------- โครงสร้างระดับกลุ่มงาน (global, department_id = NULL) ----------
$gc_rows = $conn->query("SELECT * FROM department_contents WHERE department_id IS NULL AND section = 'org_structure' ORDER BY sort_order ASC, id ASC")->fetchAll(PDO::FETCH_ASSOC);
    ...
    <div class="row justify-content-center">
        <div class="col-12">
            <div class="structure-frame">
                <?php if ($DEPT_ID && !empty($bySection['structure'])): ?>
                    <?php foreach ($bySection['structure'] as $row): ?>
                        <?php if (!empty($row['title'])): ?><div class="frame-title"><?= htmlspecialchars($row['title']) ?></div><?php endif; ?>
